...
function FillInStoryPage({ exercises, backPath }: { exercises: FillInStoryExercise[]; backPath: string }) {
  ...
  const handleSelect = (blankIndex: number, dropdownIndex: number, value: string) => {
    const key = `${blankIndex}-${dropdownIndex}`;
  ...
  const handleCheck = () => {
    setIsChecked(true);
    let correctCount = 0;
    blanks.forEach((segment, blankIdx) => {
      const expectedAnswers = segment.correctAnswers || [];
      let allCorrect = true;
      expectedAnswers.forEach((expected, dropdownIdx) => {
        const key = `${blankIdx}-${dropdownIdx}`;
  ...
  const allAnswered = blanks.every((segment, blankIdx) => {
    const expectedCount = segment.correctAnswers?.length || 0;
    for (let i = 0; i < expectedCount; i++) {
      if (!answers[`${blankIdx}-${i}`]) return false;
    }
    return true;
  });
  ...
  return (
    <div>
      ...
      <div className="bg-secondary/5 border border-secondary/20 rounded-xl p-4 mb-6">
        <p className="text-secondary font-medium">Fill in each blank with the correct pronoun(s). Some blanks need two pronouns.</p>
      </div>
          ...
          {current.segments.map((segment, idx) => {
            ...
            return (
              <span key={idx} className="inline-flex gap-1">
                {expectedAnswers.map((expected, dropdownIdx) => {
                  const key = `${currentBlankIndex}-${dropdownIdx}`;
                  const isCorrect = isChecked && answers[key] === expected;
                  const isWrong = isChecked && answers[key] !== expected;
                  
                  return (
                    <select
                      key={dropdownIdx}
                      value={answers[key] || ""}
                      onChange={(e) => handleSelect(currentBlankIndex, dropdownIdx, e.target.value)}
                      disabled={isChecked}
                      className={`px-2 py-1 rounded-lg border-2 font-medium text-base transition-colors ${
                        isCorrect ? 'border-green-500 bg-green-50' :
                        isWrong ? 'border-red-500 bg-red-50' :
                        answers[key] ? 'border-primary bg-primary/5' : 'border-border'
                      }`}
                      data-testid={`dropdown-blank-${currentBlankIndex}-${dropdownIdx}`}
                    >
...
export default function CombinedObjectsExercise() {
  const [, params] = useRoute("/practice/exercises/combined-objects/:page");
  const [selectedPage, setSelectedPage] = useState<PageType | null>(
    (params?.page as PageType) || null
  );

  const backPath = "/practice/exercises/combined-objects";

  const handleSelectPage = (page: PageType) => {
    setSelectedPage(page);
    window.history.pushState({}, "", `/practice/exercises/combined-objects/${page}`);
  };

  const handleBack = () => {
    setSelectedPage(null);
    window.history.pushState({}, "", "/practice/exercises/combined-objects");
  };
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">Direct & Indirect Objects Together</h1>
          <p className="text-lg text-muted-foreground">
            Practice using both objects in the same sentence
          </p>
        </div>
        ...
        {!selectedPage ? (
          ...
          <MultipleChoicePage 
            exercises={combinedObjectsPage1} 
            pageNum="1" 
            instruction="Replace the direct and indirect object with the correct pronouns."
            backPath={backPath}
          />
        ) : selectedPage === "2" ? (
          <RewritePage 
            exercises={combinedObjectsPage2} 
            instruction="Rewrite each sentence by replacing the direct and indirect objects with the correct pronouns."
            backPath={backPath}
          />
        ) : selectedPage === "3" ? (
          <MultipleChoicePage 
            exercises={combinedObjectsPage3} 
            pageNum="3" 
