...
import { 
  combinedObjectsSePage1, 
  combinedObjectsSePage2, 
  combinedObjectsSePage3,
  type MultipleChoiceExercise,
  type RewriteExercise 
} from "@/data/combinedObjectsSeExercises";
...
export default function CombinedObjectsSeExercise() {
  const [, params] = useRoute("/practice/exercises/combined-objects-se/:page");
  const [selectedPage, setSelectedPage] = useState<PageType | null>(
    (params?.page as PageType) || null
  );

  const backPath = "/practice/exercises/combined-objects-se";

  const handleSelectPage = (page: PageType) => {
    setSelectedPage(page);
    window.history.pushState({}, "", `/practice/exercises/combined-objects-se/${page}`);
  };

  const handleBack = () => {
    setSelectedPage(null);
    window.history.pushState({}, "", "/practice/exercises/combined-objects-se");
  };
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground mb-2">Direct & Indirect Objects (with Se)</h1>
          <p className="text-lg text-muted-foreground">
            When le/les becomes se before lo, la, los, las
          </p>
        </div>
        ...
        {!selectedPage ? (
          ...
          <MultipleChoicePage 
            exercises={combinedObjectsSePage1} 
            pageNum="1" 
            instruction="Replace the direct and indirect object with the correct pronoun combination. Remember: le/les + lo/la/los/las = se + lo/la/los/las."
            backPath={backPath}
          />
        ) : selectedPage === "2" ? (
          <RewritePage 
            exercises={combinedObjectsSePage2} 
            instruction="Rewrite each sentence by replacing the direct and indirect objects with the correct pronoun combination."
            backPath={backPath}
          />
        ) : (
          <MultipleChoicePage 
            exercises={combinedObjectsSePage3} 
            pageNum="3" 
            instruction="Choose the correct answer that replaces the direct and indirect objects with the right pronoun combination."
            backPath={backPath}
          />
        )}
      </div>
